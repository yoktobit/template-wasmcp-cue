macro_rules! mcp_tool {
    (
        ...
        mcp_tool! {
            ...
            input: $crate::component_api::Input,
            output_schema: $output_schema,
            output: $crate::component_api::Output,
            handler: $handler,
        }
    };
    (
        ...
        impl Guest for McpHandler {
            fn list_tools(
                ...
                Ok(ListToolsResult {
                    tools: vec![Tool {
                        name: $name.to_string(),
                        input_schema: include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/", $input_schema))
                            .to_string(),
                        options: Some(ToolOptions {
                            ...
                            output_schema: Some(
                                include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/", $output_schema))
                                    .to_string(),
                            ),
            ...
            fn call_tool(
                ...
                Ok(if request.name == $name {
                    ...
                    Some(match serde_json::from_str::<$input>(args) {
                        Ok(input) => {
                            let output: $output = $handler(input);
